...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, "studentAIPlayer)")
        self.myFood = None
        self.myTunnel = None
    ...
    def getMove(self, currentState):
        ...
        #if we have a drone, occupy the enemy's tunnel
        enemyPlayer = 0
        if me == 0:
            enemyPlayer = 1
        foods = getConstrList(currentState, None, (FOOD,))
        self.enemyTunnel = getConstrList(currentState, enemyPlayer, (TUNNEL,))[0]
        ...
        if droneExists == True:
            ...
            if not(self.myDrone.hasMoved):
                path = createPathToward(currentState, self.myDrone.coords,
                                       self.enemyTunnel.coords, UNIT_STATS[DRONE][MOVEMENT])
                return Move(MOVE_ANT, path, None)
