...
class AIPlayer(Player):
    ...
    def getMove(self, currentState):
        ...
        return self.evaluateLayer(currentState, currentState.whoseTurn, 0, 2)
    ...
    def evaluateLayer(self, state, playerID, depth, maxDepth):
        if (depth == maxDepth):  # final iteration (base case)
            ...
            for move in possibleMoves:
                possibleStates.append(self.predictOutcome(state, move))  # create a state for each move

            bestValue = 0.0  # arbrarily small; best move value of the entire branch (sum of self and children)
            ...
            for state in possibleStates:  # for each state
                ...
                branchValue = self.evaluateLayer(state, state.whoseTurn, depth + 1, maxDepth) + self.evaluateState(state)
                # record best value
